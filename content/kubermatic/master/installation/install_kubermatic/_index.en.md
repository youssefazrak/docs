...
For the purpose of this document we will only need to configure a few things in the values.yaml:
...
```bash
helm upgrade --tiller-namespace kubermatic --install --values YOUR_VALUES_YAML_PATH --namespace kubermatic operator config/kubermatic-operator/
```
...
spec:
  ...
  auth:
    ...
    # this need to be the same one as in the values.yaml file
    imagePullSecret: |
      {
        "auths": {
          "quay.io": {....}
